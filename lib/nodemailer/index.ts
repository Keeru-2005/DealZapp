...
export const sendEmail = async (emailContent: EmailContent, sendTo: string[]) => {
    const transporter = nodemailer.createTransport({
        pool: true,
        service: 'hotmail',
        port: 2525,  // try using port 587 or 465
        auth: {
            user: 'keeru0000@outlook.com',
            pass: process.env.EMAIL_PASSWORD,
        },
    ...
    console.log(`Sending email to ${sendTo.join(", ")} with subject "${emailContent.subject}"`);
